...
public class D2Part2 : MonoBehaviour
{
    ...
    void Start () {
        ...
        foreach(object o in spreadSheet)
        {
            ...
            {
                int testCandidate = rowValues[i];
                int matchCandidate;

                //for each cell test against all other cells in row to see if it divides evenly
                for (int j=0;j<rowValues.Count;j++)
                {
                    if (i!=j) // make sure a number doesn't try to modulo with itself
                    {
                        matchCandidate = rowValues[j];
                        int result = testCandidate % matchCandidate;

                        if (result == 0) // only a modulo result of zero matches the criteria we're looking for
                        {
                            int value = testCandidate / matchCandidate;
